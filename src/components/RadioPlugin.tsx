...
export function RadioPlugin({ url, title, editMode, onRemove }: RadioPluginProps) {
  ...
  return (
    <div 
      className="flex items-center gap-3 px-4 py-2 bg-black/40 hover:bg-black/60 border border-white/10 rounded-2xl backdrop-blur-xl transition-all shadow-xl group min-w-[280px]" 
    ...
    >
      ...
      <div className="flex flex-col min-w-0 flex-1 overflow-hidden select-none cursor-default pr-2 w-28">
      ...
      <div 
        className="flex items-center gap-2 border-l border-white/10 pl-3 py-1 ml-auto flex-shrink-0"
        onPointerDown={(e) => e.stopPropagation()} 
      >
        <button 
        ...
        <div className="w-16 relative flex items-center group/slider h-full">
           <input 
             type="range" 
